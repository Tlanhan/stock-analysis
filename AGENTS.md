# AGENTS.md — A股交易分析工作区（权威源）

> 本文件是所有 AI 代理（Claude Code / OpenCode / Codex 等）的**唯一权威操作规则**。
> `CLAUDE.md` 补充 Claude Code 特有的三阶段工作流细节，操作规则以本文件为准。

---

## 项目性质

**不是软件项目** — 无 build/test/package.json/pyproject.toml。这是个人 A 股交易分析与投研工作区，产出物为 Markdown 报告、HTML 可视化页面和 Python 工具脚本。

---

## 路径占位符（跨机器/跨平台兼容，2026-06-23 新增）

> 为消除绝对路径硬编码，本工作区所有文档统一使用以下占位符。AI 读取时按本表解析为实际路径。

| 占位符 | 解析为（本机默认） | 说明 |
|--------|------------------|------|
| `{{WORKSPACE_ANALYSIS}}` | `G:/github/stock-trading-system/stock-analysis` | 本仓库（试炼场/工作区）根目录 |
| `{{WORKSPACE_SKILL}}` | `G:/github/stock-trading-system/stock-skill` | AI Skill 长期归档仓库 |
| `{{WORKSPACE_JOURNAL}}` | `G:/github/stock-trading-system/stock-journal` | 复盘文件终态归档仓库 |
| `{{SKILLS_ACTIVE}}` | `~/.claude/skills` | AI 生效环境（skill 软链接目录） |

> 换机器/换目录时，只需修改本表四处，全工作区文档自动适配。

---

## 关键工具

| 工具 | 用法 | 注意 |
|------|------|------|
| `tools/qt_quote.py` | `python tools/qt_quote.py 600183 002916` 或 `python tools/qt_quote.py @watchlist.txt` | 腾讯 qt.gtimg.cn 接口，A股/指数行情，支持 `--json`/`--csv` |
| `trade-mistakes/sync-to-issue.sh` | `./sync-to-issue.sh YYYY-MM-DD-简述.md [--dry-run]` | 需 `gh` CLI 已登录 `Tlanhan/stock-journal` |

### 数据源限制

- `kimi-datasource` (`query_stock`)：**不支持 ETF 查询**
- `a-stock-analysis skill`（新浪接口）：深交所新 ETF（如 159558）可能无法获取
- `qt_quote.py`：当前最稳定的行情源，无风控，批量快

---

## 文件命名约定

| 类型 | 模式 | 示例 |
|------|------|------|
| 每日复盘 | `YYYY-MM-DD-daily-review.md` | `2026-06-16-daily-review.md` |
| 盘中复盘 | `YYYY-MM-DD-intraday[-HHmm].md`（HHmm 可选，仅同日多份时加）| `2026-06-16-intraday.md`（单份）或 `2026-06-16-intraday-1130.md`（多份）|
| 周度复盘 | `YYYY-MM-DD-weekly-review.md` | `2026-06-12-weekly-review.md` |
| 交易卡片 | `YYYY-MM-DD-trading-card.md` | `2026-06-12-trading-card.md` |
| 条件单卡 | `YYYY-MM-DD-条件单设置卡[-vN].md` | `2026-06-16-条件单设置卡-v2.md` |
| 情绪日历 | `sentiment-calendar-YYYY-MM.md` | `sentiment-calendar-2026-06.md` |
| 海外催化 | `oversea-catalyst-YYYY-WNN.md` | `oversea-catalyst-2026W23.md` |
| 错误档案 | `YYYY-MM-DD-简述.md`（4-8字中文） | `2026-06-02-COMPUTEX错过.md` |

> 周数算法：ISO 8601，`date +%V` 验证。

---

## 复盘文件路径（非本目录）

**复盘终态归档在 `{{WORKSPACE_JOURNAL}}/`，不在本目录。**

| 类型 | 写入路径 |
|------|---------|
| 日复盘 | `{{WORKSPACE_JOURNAL}}/reviews/daily/YYYY-MM/YYYY-MM-DD.md` |
| 盘中复盘 | `{{WORKSPACE_JOURNAL}}/reviews/daily/YYYY-MM/YYYY-MM-DD-intraday[-HHmm].md`（HHmm 可选） |
| 周复盘 | `{{WORKSPACE_JOURNAL}}/reviews/weekly/YYYY-WNN.md` |

写入前 `mkdir -p`，写完提醒用户"可推送"，用户说"推送复盘"才 `git commit + push`。

---

## 交易错误档案

- 草稿区：`trade-mistakes/YYYY-MM-DD-简述.md`（从 `TEMPLATE.md` 复制）
- 终态：**GitHub Issue**（`Tlanhan/stock-journal`），不是文件
- 用户说"推送错误"/"开 issue" → `sync-to-issue.sh` → 成功后回写 issue 号到本地 md
- **错误类型**：E1 追涨杀跌 / E2 止损不严 / E3 情绪化开仓 / E4 规则外仓位 / E5 错过机会 / E6 提前出场 / E7 加错仓
- **默认不推 GitHub**（含交易金额/仓位敏感信息）

---

## 强制规则（会搞错的高频坑）

### 0. 会话启动必须校准"今天 + 本周 + 本月"（最高优先级前置层）

**每次会话开始的第一件事**，不是回应用户问题，而是先用 Python 确认并**明确告知用户**三件事：
1. **今天**：几月几号、星期几、当前北京时间几点、A 股交易时段状态
2. **本周范围**：本周一到周日的完整日期范围（ISO 周数），以及本周已发生/未发生的交易日
3. **本月范围**：本月已过几个交易日、本月剩余交易日

这是所有后续讨论的时间基准。**只校准"今天"不够**——用户问"本周行情"时，必须知道本周从哪天开始，不能被对话上下文锚定。

```python
from datetime import datetime, timezone, timedelta
bj = timezone(timedelta(hours=8))
now = datetime.now(bj)
iso = now.isocalendar()
# 本周一/周日
monday = now - timedelta(days=now.weekday())
sunday = monday + timedelta(days=6)
print(now.strftime('%Y-%m-%d %A %H:%M:%S'))
print(f'本周: {monday.strftime("%m/%d")}(周一) ~ {sunday.strftime("%m/%d")}(周日), ISO week {iso[1]}')
print(f'本月: 6月, 已过交易日需逐一核验')
# 必须输出且告知用户
```

**为什么必须校准本周/本月范围**：
- **已发生事故（2026-06-20）**：用户问"深度总结这周行情"，AI 因上下文锚定（对话整段在讨论 6/16-6/18 FOMC 三天），把"本周"当成 6/16-6/18 三天，**漏掉了 6/15 周一这个本周真正的爆发起点**（创业板+5.3% 破 4000）。根因：只校准了"今天=6/20"，没有推导"本周完整范围=6/15~6/21"，被对话历史锚定。
- AI 跨会话/跨对话会丢失日历事实，把"对话讨论过的日期"当成"日历范围"
- 用户说"这周/上周/下周/这个月"时，**必须先用代码算出绝对日期范围**，禁止用对话上下文的日期当边界

**校准输出模板**（每次会话首条回复必须包含）：
> ✅ 已校准：**今天 2026-MM-DD 周X HH:MM**（北京时间），A 股状态：[盘前/盘中/盘后/休市]。
> 📅 本周：MM/DD(周X) ~ MM/DD(周X)，ISO-WNN，交易日：[列出]。
> 📆 本月：已过 N 个交易日。

**附加要求**：
- 用户说"本周/上周/本月"等相对时间词 → **立即用 Python 推导绝对范围**，禁止用对话上下文
- 校准后再判断数据可得性（盘后才有收盘数据，盘前只能用昨日数据并标注）
- 用户消息提及具体日期 → 用代码核验星期，禁止凭记忆
- 距上次会话 >1 天 → 主动提醒"情绪日历/海外催化文件可能需要刷新"

### 0.5 分析/交易决策前必须先拉 A 股一手真值（2026-06-29 事故新增）

> **背景**：2026-06-29 用户问"半导体设备是否超买"，AI 用 6/26（上周五）的缓存数据（4.147）当 6/29 实时数据分析，算出"乖离+6.78%"。**真相是 6/29 当天 159558 +8.97% 收 4.519，实际超买程度远超分析值**。用户当场指出"分析有点失真"。根因：只校准了时间，没有拉取当下 A 股行情，用旧数据当新数据。

**铁律**：时间校准（第0条）完成后，做任何 A 股分析/交易决策前，**必须先拉取当前 A 股一手真值**，再开始分析。

**强制执行流程**：
```
Step 1: 完成第0条时间校准（北京时间 + 交易状态）
   ↓
Step 2: 判断 A 股状态
   ├─ 盘中（9:30-15:00）→ 拉实时行情
   ├─ 盘后（15:00-）→ 拉当日收盘数据
   └─ 盘前（-9:30）→ 拉昨日收盘，明确标注"昨日数据"
   ↓
Step 3: qt_quote 拉四大指数 + 用户标的
   python tools/qt_quote.py sh000001 sz399001 sz399006 sh000688 <用户标的>
   ↓
Step 4: em_data 拉板块/资金（交易决策时）
   python tools/em_data.py sector
   ↓
Step 5: 数据带日期归属标注，再开始分析
   "数据获取时间：北京时间 YYYY-MM-DD HH:MM（盘后收盘）"
```

**禁止行为**：
- ❌ 用上次会话拉的数据当本次分析依据（数据可能跨日，已过期）
- ❌ 用对话历史里的价格当现价（如上文提到 4.147，本次又引用）
- ❌ 只校准时间不拉数据，直接开始分析
- ❌ 拉了数据但不说"这是哪天的数据"（必须标注日期归属）

**为什么必须每次重新拉**：
- A 股每日波动大，昨日收盘 ≠ 今日现价（159558 6/26 收 4.147 → 6/29 收 4.519，差 +9%）
- AI 跨会话/跨对话会丢失数据新鲜度，把"上次拉的数据"当"现在的数据"
- 交易决策建立在过期数据上 = 高位接盘 / 低位割肉

### 1. 时间必须用 Python UTC+8

系统时区可能配置错误，**禁止依赖 `date` 命令**。

```python
from datetime import datetime, timezone, timedelta
bj = timezone(timedelta(hours=8))
print(datetime.now(bj).strftime('%Y-%m-%d %H:%M:%S %A'))
```

获取数据后必须标注：`数据获取时间：北京时间 YYYY-MM-DD HH:MM`

### 2. 日期+星期必须用代码核验

**禁止凭印象写星期。** AI 多次把"6/16 周二"写成"6/16 周一"。

```python
from datetime import datetime
print(datetime(2026,6,17).strftime('%Y-%m-%d %A'))
# 2026-06-17 Wednesday → 才能写"6/17 周三"
```

### 3. 各市场交易时段（北京时间）

| 市场 | 交易时段 |
|------|---------|
| A股 | 周一-周五 9:30-11:30, 13:00-15:00 |
| 港股 | 周一-周五 9:30-12:00, 13:00-16:00 |
| 美股（夏令时 3月中-11月初） | **21:30**-次日4:00 |
| 美股（冬令时 11月初-3月中） | **22:30**-次日5:00 |

### 4. 消息真实性三层检验

任何消息驱动型交易决策必须过三层：

1. **双日期标注**：报道日期 + 事件首发日期同时标注
2. **旧闻翻炒识别**：跨源搜索找首发日期，>30天 → ❌ 旧闻翻炒
3. **借利好出货反向判定**：5个出货信号叠加，2+ → 🚨 出货警报

### 5. trigger-check 强制调用

用户表达交易意图时**必须先调用 `a-share-trigger-check`**，不允许用分析替代执行框架。

| 用户意图 | 强制模式 | 输出 |
|---------|---------|------|
| "我想买/我要做/可以做吗" + 明确标的 | 模式A 写条件交易卡 | IF-THEN 条件卡（价格/仓位/止损）|
| "应该买吗/我有感觉/现在能进吗" | 模式B 扳机检查 | 30秒 GO/NO-GO |
| "后悔没做/想做但没做/错过了/踏空" | 模式C 失误复盘 | 结构化归因 |
| "缩量/量价/低开" | 模式I 量价触发 | 四重确认 + 量价观察清单 |
| "明天怎么做/明日策略/写卡" | 模式A + 情绪日历 + 催化 | 组合调用 |

**核心铁律**：执行框架优先于分析。用户说"想交易"时，**先出卡，再补数据支撑**。不允许用分析对话替代执行框架。

### 6. 情绪扰动日历强制加载

- **文件**：`sentiment-calendar-YYYY-MM.md`（本目录）
- **触发**：用户问"明天怎么做"/"写卡"/做任何交易决策/触发复盘时
- **失效检查**：当月文件不存在 → 提示生成；距今 >25 天 → 提示刷新
- **写卡铁律**：检查"明日"是否在 🔴 强扰动日中，若在则加入"不做清单"+ 降仓建议

### 7. 海外科技催化主动扫描

- **文件**：`oversea-catalyst-YYYY-WNN.md`（本目录）
- **触发**：用户问海外/美股/英伟达/苹果等关键词；写卡/盘前规划时
- **失效检查**：当周文件不存在 → 提示扫描；距今 >5 天 → 提示刷新
- **刷新触发词**："更新催化"/"刷新催化"/"更新海外"/"OST 扫描"

### 8. 综合分析流程（交易决策时）

```
用户问"是否应该交易"
  → [第1步] a-share-trigger-check（执行框架优先）
  → [第2步] 加载情绪日历 + 海外催化（扰动检查）
  → [第3步] a-share-technical-analysis / a-stock-analysis（数据支撑）
  → [输出] 执行框架 + 扰动检查 + 数据支撑 = 综合决策
```

每个 skill 的输出必须嵌入最终回复，不能只说"我调用了 skill"。

### 9. 协作节奏：市场真值前置（2026-06-20 新增）

> **背景**：用户深度复盘本周（W25）踏空，核心痛点是——①进场由恐惧驱动（怕买错撒小单）；②持仓被震荡洗出（周二开盘全卖）；③同时盯两个标的导致"脑子宕机"；④预测式框架失灵时空仓一整天。用户明确指出：**纯粹的情绪管理会脱离市场真实情况**，必须"基于消息面、走势、情绪"三者结合。
>
> **本节定义 AI 与用户的协作节奏**：AI 不是替用户预测市场，是在用户最容易犯错的三个时刻（盘前/盘中犹豫/盘后），先用**一手市场真值**把市场事实钉死，再陪用户审视决策。**顺序铁律：先拉真实数据陈述事实，再做情绪审视，禁止空对空地问"你觉得呢"。**

#### 9.1 三时刻协作节奏

| 时刻 | 用户触发 | AI 必做（按顺序）|
|------|---------|----------------|
| **盘前 9:00-9:15** | "今天想做XX" / "写卡" | ① 拉 qt_quote 昨收+盘前竞价 ② em_data 板块/资金 ③ WebSearch 消息面 ④ 用 trigger-check 模式A 写条件卡，**强制含三行**：主盯标的(仅1个)/挂单标的(条件触发)/加仓上限(金额+次数) |
| **盘中犹豫时** | "蒙了/想卖/想追/要不要动" | ① **先拉真实数据**（qt_quote 四指数+用户标的实时价 + em_data 板块谁在涨+主力净流入 + 消息面）② **摆事实不解释**（"创业板+1.2% 沪指-0.3% 剪刀差1.5%，半导体主力净流入28亿排第3，你的159558现3.51"）③ 用 trigger-check 模式识别层四问引导归类（事实→分类→应对→自检）④ 用户回答后归到操作 |
| **盘后 15:00 后** | "今天做了XX" / "复盘" | ① 拉当日完整收盘数据 ② 帮用户归因（不评判）③ 对照用户交易短板（恐惧驱动进出场/脑子宕机/预测框架失灵）看本次是否重犯 |

#### 9.2 数据获取三层优先级（方法论铁律，2026-06-23 新增）

> **背景**：6/23 查中信期货持仓时，API 未打通就直接跳到 WebSearch 拼新闻，把 MCP 兜底跳过了。更早还发生过：用昨日 A 股收盘当今天数据、用昨日期货收盘当今天利空。根因是数据获取无优先级，随手抓到什么用什么。

**数据获取必须严格按以下优先级，不可跳级：**

| 优先级 | 方式 | 适用数据 | 说明 |
|:---:|------|---------|------|
| **1（首选）** | **一手 API** | 价格、持仓、资金、K线、板块、融资 | `qt_quote.py` / `em_data.py` / 东财 datacenter / 腾讯接口。有结构化、无滞后、可校验 |
| **2（兜底）** | **MCP webReader** | API 不可用时的降级 | 渲染 SPA 页面，能抓动态数据（如期指龙虎榜）。**API 不行才用，不是首选** |
| **3（仅限消息面）** | **WebSearch** | 消息面、政策、事件、新闻 | **仅限本就没有 API 的领域**。价格/持仓/资金有 API 的，禁止用 WebSearch 拼新闻冒充 |
| **4（底线）** | **如实告知"无数据"** | 以上都不行时 | **禁止编造、禁止用旧数据冒充新数据、禁止用新闻二手值冒充一手**。查不到就说查不到，用户宁愿知道"没有"也不愿被假数据误导 |

**三条硬约束：**
1. **不可跳级**：API 不行 → MCP → 无数据。禁止从 API 直接跳到 WebSearch
2. **期货数据盘前必拉当天实时**：期货 9:00 开盘，股票盘前预期时必须拉当天期货（碳酸锂/铜/黄金等），禁止用昨日收盘冒充今天。`225.lc{月份}` 为碳酸锂 secid（广期所）
3. **数据带日期归属**：任何数据必须标注"哪个交易日的"，禁止模糊。用昨天的就说"截至X日"，用今天的就说"X日实时"

#### 9.3 市场真值前置的工具栈（每时刻必用）

| 数据 | 工具 | 何时拉 |
|------|------|--------|
| 实时行情（指数/ETF/个股）| `tools/qt_quote.py` | 每次响应前 |
| 板块涨幅+主力净流入 | `tools/em_data.py sector` | 盘中犹豫/写卡时 |
| 融资余额（资金面）| `tools/em_data.py margin` | 写卡/深度分析时 |
| 消息面/政策 | WebSearch（财联社/证券时报）| 写卡/事件驱动时 |
| 海外映射（美股/费半/美债/A50）| qt_quote + WebSearch | 写卡/事件驱动时 |
| 情绪日历扰动日 | `sentiment-calendar` skill | 写卡时强制 |
| 海外催化 | `oversea-catalyst` skill | 涉及海外时 |

> ⚠️ **禁止空对空审视**：用户说"蒙了"时，AI 不能直接问"你觉得呢"。必须先用上表工具拉真值，**陈述完市场事实**，再引导审视。脱离市场真实的情绪管理=心理辅导，不是交易辅助。

#### 9.4 用户六大交易短板的针对性拦截（2026-07-16 新增第6项）

AI 在协作中须主动对照用户已识别的短板，在对应时刻拦截：

| 短板 | 发作时刻 | AI 拦截方式 |
|------|---------|------------|
| **进场撒小单**（恐惧驱动）| 盘前/开盘 | 条件卡强制写"首批固定仓位"，不给盘中留"买多少"的决策窗口 |
| **持仓被洗**（怕回吐开盘卖）| 盘中想卖时 | 先问"触发作废条件了吗？没有=这是情绪还是逻辑？"，给30秒缓冲；补一条规则"突破次日不在前30分钟做卖出决定" |
| **同时盯多标的宕机** | 进场时 | 条件卡强制"主盯仅1个（选波动大的）+其余挂单"，禁止同时盯2个以上 |
| **预测框架失灵空仓** | 盘中偏离预期时 | 用模式识别层（事实→分类）替代预测，识别"突破后低开=回踩买点"而非"方向不明" |
| **越跌越买过度集中** | 加仓时 | 条件卡强制写"单票总仓位上限+加仓次数上限（默认2次）"，到顶不再加 |
| **开仓后逐一补仓**（计划外加仓推高成本）| 想加仓时 | 对照**建仓计划卡**（铁律八），计划外加仓请求直接拒绝（"这不在你开盘前写的建仓计划卡内"）；若用户坚持，要求先说明"出现了什么计划外的新客观信号"，而非"信心不足/已经买了"。详见 [Issue #17](https://github.com/Tlanhan/stock-journal/issues/17)、`a-share-trigger-check` 加仓前五问第0问 |

---

---

## Git 提交规范

- Commit message：中文描述性，禁止 `update`/`fix` 空话
- 示例：`新增：6/16 A股每日复盘报告` / `认知迭代：有色与科技跷跷板效应`
- Atomic commit，每个独立逻辑一个 commit
- **推送需用户明确触发**：用户说"推送"/"push"才执行

---

## 相关仓库

| 仓库 | 路径 | 用途 |
|------|------|------|
| stock-skill | `{{WORKSPACE_SKILL}}/` | AI Skill 长期归档（`{{SKILLS_ACTIVE}}/` → 此仓库） |
| stock-journal | `{{WORKSPACE_JOURNAL}}/` | 复盘文件终态归档 + 交易错误 Issue |
| 本仓库 | `{{WORKSPACE_ANALYSIS}}/` | 试炼场（讨论、草稿、工作数据） |

---

## 安全

- `.claude/settings.local.json` 含 MCP 认证信息，已被 `.gitignore` 忽略
- 数据获取失败时标注"未能确认"，不用旧数据填空
- 所有分析报告必须包含"不构成投资建议"声明

---

*2026-06-18 重写；2026-06-18 增补第0条"会话启动校准今天"铁律（修复跨日数据混淆事故）。详细工作流（三阶段分离、情绪日历加载、海外催化扫描等）见 `CLAUDE.md`。*
